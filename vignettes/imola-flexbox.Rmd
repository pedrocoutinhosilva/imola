---
title: "CSS Flexbox and imola"
output: rmarkdown::html_vignette
description: >
  Getting started with CSS Flexbox and imola.
vignette: >
  %\VignetteIndexEntry{CSS Flexbox and imola}
...
# Available arguments
The main functions used to generate flex containers are `flexPanel()` and `flexPage()`. There's a lot of arguments going on here, so lets go over what each option can do:
...
```
![two-three-alternate layout](../reference/figures/two-three-alternate-preview.png)
...
![](../reference/figures/flex-direction.svg)
...
![](../reference/figures/flex-wrap.svg)
...
![](../reference/figures/justify-content.svg)
...
![](../reference/figures/align-items.svg)
...
![](../reference/figures/align-content.svg)
...
By default the value `c(1)` is used, meaning a value of `1` for every child. You might notice that this is diferent from the pure css default, but makes more sense in the context of shiny dashboards.
...
![](../reference/figures/flex-grow.svg)
...
![](../reference/figures/flex-shrink.svg)
...
![](../reference/figures/flex-basis.svg)
...
When set to true this will force the grid to be at least as tall as the available browser window. This makes the container stretch in the content is smaller, while still allowing it to grow behond the height of the viewport if necessary.

...
Using imola can be as simple as using a template in your dashboard, or as deep as building a fully custom layout, complete with diferent views for diferent screen sizes.

In addition to the convenient wrapper functions for CSS Flex and CSS Grid, imola also includes support for dealing with diferent screen sizes as well as saving and reusing layouts via the concept of `breakpoint systems` and `templates`.
...
While in depth knowledge of CSS Flexbox and CSS Grid is not required to get started, it is recomended to at least be somewhat familiar with them. I recommend [this article](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Grid_Layout/Relationship_of_Grid_Layout) to get you up to speed.
...
Picking between grid and flex might seem tricky. In general flex and Grid serve different purposes and are both incredibly useful. Picking between them tipically depends on your use case:
...
Lets say we would like to save our previous areas as a template, and use it in diferent components. We could use `gridTemplate()` to generate a template object:

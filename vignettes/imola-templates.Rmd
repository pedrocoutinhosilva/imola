...
After a template being registered, you can then simple fill in the `template` argument on any of the panel or page functions with the name of the template. You can also adjust the template for a specific panel by providing any named arguments in addition to the template name. Any named argument that exists in the template will be overwritten by the named argument on the function call.
...
You can also create templates as R objects and feed that object to the `template` argument instead, this can be useful if you do not want to make a template globally accessible (For example for usage only in a single module, or for generating this programmatically based on other factors).

In that case, you can use the `gridTemplate()` function in a similar way to `registerTemplate()`, but assign it to an R object (or invoke it directly on the `template` argument).
``` r
templateObject <- gridTemplate("grid",
  areas = list(
    default = c(
      "area1 area1 area1",
      "area2 area3 area3",
